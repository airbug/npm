...
function addLocalTarball (p, name, absoluteSymlinks, cb_) {
  if (typeof cb_ !== "function") cb_ = absoluteSymlinks, absoluteSymlinks = false
  if (typeof cb_ !== "function") cb_ = name, name = ""
  // if it's a tar, and not in place,
  // then unzip to .tmp, add the tmp folder, and clean up tmp
  if (p.indexOf(npm.tmp) === 0) return addTmpTarball(p, name, absoluteSymlinks, cb_)
  ...
  mkdir(path.dirname(tmp), function (er) {
    ...
    to.on("close", function () {
      ...
      fs.chmod(tmp, npm.modes.file, function (er) {
        if (er) return cb(er)
        addTmpTarball(tmp, name, absoluteSymlinks, cb)
      })
    })
    from.pipe(to)
  })
...
function addLocalDirectory (p, name, absoluteSymlinks, cb) {
      ...
      mkdir(path.dirname(tgz), function (er, made) {
        ...
        tar.pack(tgz, p, data, doFancyCrap, absoluteSymlinks, function (er) {
          ...
          chownr(made || tgz, cs.uid, cs.gid, function (er) {
            if (er) return cb(er)
            addLocalTarball(tgz, name, absoluteSymlinks, cb)
          })
        })
      })
    })
...
function addTmpTarball (tgz, name, absoluteSymlinks, cb) {
  if (typeof cb !== "function") cb = absoluteSymlinks, absoluteSymlinks = false
  if (!cb) cb = name, name = ""
  getCacheStat(function (er, cs) {
    if (er) return cb(er)
    ...
    tar.unpack( tgz, path.resolve(contents, "package")
      ...
      addLocalDirectory(path.resolve(contents, "package"), name, absoluteSymlinks, cb)
    })
  })
}
