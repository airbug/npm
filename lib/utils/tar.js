...
function pack (tarball, folder, pkg, dfc, absoluteSymlinks, cb) {
  ...
  if (dfc) {
    ...
    return lifecycle(pkg, "prepublish", folder, function (er) {
      if (er) return cb(er)
        pack_(tarball, folder, pkg, absoluteSymlinks, cb)
    })
  } else {
    pack_(tarball, folder, pkg, absoluteSymlinks, cb)
  }
...
function pack_ (tarball, folder, pkg, absoluteSymlinks, cb_) {
  ...
  function next () {
    ...
    new Packer({ path: folder, type: "Directory", isDirectory: true })
      ...
      .pipe(tar.Pack({ noProprietary: !npm.config.get("proprietary-attribs") }, {absoluteSymlinks: absoluteSymlinks}))
      .on("error", function (er) {
