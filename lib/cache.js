...
// npm cache add <folder>
cache.add = function (pkg, ver, scrub, absoluteSymlinks, cb) {
  if (typeof cb !== "function") cb = absoluteSymlinks, absoluteSymlinks = false
  if (typeof cb !== "function") cb = scrub, scrub = false
  if (typeof cb !== "function") cb = ver, ver = null
  if (scrub) {
    return clean([], function (er) {
      if (er) return cb(er)
      add([pkg, ver], absoluteSymlinks, cb)
    })
  }
  log.verbose("cache add", [pkg, ver])
  return add([pkg, ver], absoluteSymlinks,cb)
}

function add (args, absoluteSymlinks, cb) {
  ...
  if (!name && !p.protocol && spec.indexOf("@") !== -1) {
    ...
    name = spec.shift()
    spec = spec.join("@")
    return add([name, spec], absoluteSymlinks, cb)
  }
  ...
  switch (p.protocol) {
    ...
    default:
      ...
      if (name) {
        ...
        addLocal(spec, "", absoluteSymlinks, cb)
      }
  }
}
...
function addLocal (p, name, absoluteSymlinks, cb_) {
  if (typeof cb_ !== "function") cb_ = absoluteSymlinks, absoluteSymlinks = false
  if (typeof cb_ !== "function") cb_ = name, name = ""
  ...
  lock(p, function (er) {
    ...
    fs.stat(p, function (er, s) {
      ...
      if (s.isDirectory()) addLocalDirectory(p, name, absoluteSymlinks, cb)
      else addLocalTarball(p, name, absoluteSymlinks, cb)
    })
  })
}
...
function addLocalTarball (p, name, shasum, absoluteSymlinks, cb_) {
  if (typeof cb_ !== "function") cb_ = absoluteSymlinks, absoluteSymlinks = false
  if (typeof cb_ !== "function") cb_ = shasum, shasum = null
  if (typeof cb_ !== "function") cb_ = name, name = ""
  ...
  if (p.indexOf(npm.tmp) === 0)
    return addTmpTarball(p, name, shasum, absoluteSymlinks, cb_)
  ...
  mkdir(path.dirname(tmp), function (er) {
    ...
    to.on("close", function () {
      ...
      fs.chmod(tmp, npm.modes.file, function (er) {
        if (er) return cb(er)
          addTmpTarball(tmp, name, shasum, absoluteSymlinks, cb)
      })
    })
    from.pipe(to)
  })
...
function addLocalDirectory (p, name, shasum, absoluteSymlinks, cb) {
  ...
  readJson(path.join(p, "package.json"), function (er, data) {
    ...
    getCacheStat(function (er, cs) {
      mkdir(path.dirname(tgz), function (er, made) {
        if (er) return cb(er)
        tar.pack(tgz, p, data, doFancyCrap, absoluteSymlinks, function (er) {
          if (er) {
          ...
          chownr(made || tgz, cs.uid, cs.gid, function (er) {
            if (er) return cb(er)
              addLocalTarball(tgz, name, shasum, absoluteSymlinks, cb)
          })
        })
      })
    })
...
function addTmpTarball (tgz, name, shasum, absoluteSymlinks, cb) {
  if (typeof cb !== "function") cb = absoluteSymlinks, absoluteSymlinks = false
  if (!cb) cb = name, name = ""
  getCacheStat(function (er, cs) {
    if (er) return cb(er)
    ...
    tar.unpack( tgz, path.resolve(contents, "package")
      ...
      addLocalDirectory(path.resolve(contents, "package"), name, shasum, absoluteSymlinks, cb)
    })
  })
}
